            ...
            function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue, callback)
                ...
                sliderBtn.MouseButton1Down:Connect(function()
                    if not focusing then
                        ...
                        Value = (math.floor(((((tonumber(maxvalue) - tonumber(minvalue)) / 149) * sliderDrag.AbsoluteSize.X) + tonumber(minvalue)) * 100) / 100) or 0
                        pcall(function()
                            callback(Value)
                        end)
                        ...
                        moveconnection = mouse.Move:Connect(function()
                            val.Text = Value
                            Value = math.floor(((((tonumber(maxvalue) - tonumber(minvalue)) / 149) * sliderDrag.AbsoluteSize.X) + tonumber(minvalue)) * 100) / 100
                            pcall(function()
                                callback(Value)
                            end)
                            sliderDrag:TweenSize(UDim2.new(0, math.clamp(mouse.X - sliderDrag.AbsolutePosition.X, 0, 149), 0, 6), "InOut", "Linear", 0.05, true)
